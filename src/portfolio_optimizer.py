...
from gerber import gerber_cov_stat1, gerber_cov_stat2, dynamic_gerber_cov
...
class portfolio_optimizer:
    def __init__(self, min_weight: float = 0., max_weight: float = 1.0,
                 ...
                 dynamic_threshold: np.array = np.full((9,), 0.5)):
        """
        :param min_weight:
        :param max_weight:
        ...
        assert np.all(dynamic_threshold >0),"The Gerber shrinkage threshold shall be non-negative"
...
# NEW: Dynamic Gerber Covariance Calculation

        elif self.cov_function == "GS2":
#             print(self.dynamic_threshold)
            self.covariance, _ = dynamic_gerber_cov(self.returns_df.values, threshold=self.dynamic_threshold)
            self.covariance_neg, _ = dynamic_gerber_cov(self.negative_returns_df.values, threshold=self.dynamic_threshold)

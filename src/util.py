...
def calc_assets_moments(returns_df: pd.DataFrame,
    ...
    if weights is None:
        ...
        if cov_function is None:
            ...
            cov_mat, _ = dynamic_gerber_cov(returns_df.values)  # covariance matrix
...
def get_frontier_limits(returns_df: pd.DataFrame,
                        ...
                        dynamic_threshold: np.array = np.full((9,), 0.5)) -> dict:
    """
...
def get_frontier_by_return(returns_df: pd.DataFrame,
    ...
    """
    port_opt = portfolio_optimizer(min_weight=0, max_weight=1, cov_function=cov_function, freq=freq,gs_threshold=gs_threshold,
                                               dynamic_threshold=dynamic_threshold)
...
def get_frontier_by_risk(returns_df: pd.DataFrame,
    ...
    _port_limits = get_frontier_limits(returns_df, cov_function, freq, gs_threshold=gs_threshold,dynamic_threshold=dynamic_threshold)

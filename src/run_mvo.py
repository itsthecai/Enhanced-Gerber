...
lookback_win_size = 12 * lookback_win_in_year # Look back window in months


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Parse parameter for Bloomberg 9")
    parser.add_argument("-s", "--gs_threshold", type=float, default=0.5) # UPDATE: this is initial threshold
    ...
    # NEW: initialize Dynamic Gerber constants
    gerber_c = pd.DataFrame(np.full( prcs.shape , gs_threshold),columns=prcs.columns, index=prcs.index)

    std_window = 24
    rolling_std = rets.rolling(window=std_window).std()

    # calculate the historical risk ratio 
    risk_ratio = rolling_std.div(rolling_std.mean(axis=1),axis=0) 
    # calculate the Exponential moving average of the risk ratio
    risk_bar = risk_ratio.ewm(span=4, adjust=False).mean()
    ...
    t = lookback_win_size # t is the current time
    ...
    for t in tqdm(range(lookback_win_size, nT)):
        ...
        # NEW: Update gs-threshold HERE
        
        # Calculate risk ratio at time t [S_ti: std of asset i at time t; S_t: std of the all assets at time t]
        rho_t = sub_rets.std() / (sub_rets.std().mean())

        # Retrieve historical risk ratio
        risk_bar_t = risk_bar.iloc[t-1]
        
        # previous gs-threshold 
        gs_threshold_m1 = gerber_c.iloc[t-1]

        # new gs-threshold 
        gs_threshold_t = (rho_t / risk_bar_t) * gs_threshold_m1
        gerber_c.iloc[t] = gs_threshold_t
        ...
        for cov_function in cov_function_list:
            ...
            # NEW: optimization with new Gerber threshold

            if cov_function == 'GS2':
#                 print(gs_threshold_t) #Checked
                print(end_date_str)
                opt_ports_dict[cov_function] = get_mean_variance_space(sub_rets,
                                                                       ...
                                                                       dynamic_threshold=gs_threshold_t)
            ...
            for port_name in port_names:
                ...
                port_t["shares"] = port_tm1['portValue'] * port_t["weights"] / prcs_t  # Budget of each asset / asset price 
